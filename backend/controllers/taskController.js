...
exports.createTask = async (req, res) => {
  ...
  const managerId = req.userId; // ID du manager connecté (depuis le middleware d'authentification)
...
exports.getUserTasks = async (req, res) => {
  try {
    const tasks = await Task.find({ assignedTo: req.userId }); // Ne récupère que les tâches de l'utilisateur connecté
...
exports.getAllTasks = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Erreur serveur lors de la récupération de toutes les tâches" });
  }
};
...
exports.updateTask = async (req, res) => {
  ...
  try {
    ...
    if (task.assignedTo.toString() !== req.userId.toString() && req.role !== 'manager') {
...
exports.deleteTask = async (req, res) => {
  ...
  try {
    ...
    if (req.role !== 'manager') {
      return res.status(403).json({ message: "Accès interdit : Vous ne pouvez pas supprimer cette tâche" });
    }
